...
public:
    ...
    bool fillByFileName(const QString &fileName);
    bool remove();
    bool isFetched();
    bool isCompleted();
    static CalendarItem fetchByUrlAndCalendar(const QString &url, const QString &calendar);
    static bool addCalendarItemForRequest(const QString &calendar, QUrl url, const QString &etag, const QString &lastModifiedString);
...
private:
    ...
    static QString decodeICSDataLine(const QString &line);
    static QString findFreeHashKey(QHash<QString, QString> *hash, const QString &key, int number = 0 );
